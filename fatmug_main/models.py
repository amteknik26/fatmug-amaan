...
class Vendor(models.Model):
    ...
    average_response_time = models.FloatField(null=True,default=None,db_comment="in Minutes")
...
class PurchaseOrder(models.Model):
    ...
    def clean(self):
        ...
        if (self.delivery_date < self.acknowledgment_date) or (self.delivery_date < self.issue_date):
            raise ValidationError("Delivery date must be equal to or after the issue date and acknowledgement date.")
        ...
        if self.quality_rating is not None and (self.quality_rating < 0 or self.quality_rating > 100):
